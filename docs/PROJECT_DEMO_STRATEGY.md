...
| `metricdrive` | Static autonomy planning explorer generated from Python experiments; no backend required. | Portfolio static route + GitHub README/release. | Yes, but the portfolio route is canonical because the custom domain is served by Vercel. | P0 | Serve `/metricdrive/`, link the repo/release, and keep the static demo synced from `docs/demo/`. |
